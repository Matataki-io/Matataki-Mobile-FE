  ...
  <div>
    ...
    <div class="kvFQhz">
      <div class="iNUelT">
        <div class="OpDFW">
          ...
          <div
            v-if="isDelete && form.outputToken.symbol && form.outputToken.id !== 0"
          >流动金Token：{{ yourPoolSize.your_supply }}</div>
        </div>
        <!--------------------- 删除流动金代码开始 ---------------------->
    ...
    <div class="kvFQhz">
      <div class="iNUelT">
        <div class="OpDFW">
          ...
          <div
            v-if="form.outputToken.symbol && form.outputToken.id !== 0 && !isDelete"
          >余额：{{ balance.add }}</div>
        </div>
        <!--------------------- 删除流动金代码开始 ---------------------->
        <div v-if="isDelete" class="cHbrWc">
          <template v-if="outputPoolSize.cny_amount !== 0">
            ...
            <div
              class="kroqsf"
            >{{ outputPoolSize.token_amount.toFixed(4) }} {{ form.outputToken.symbol }}</div>
          </template>
        </div>
        <!--------------------- 删除流动金代码结束 ---------------------->
        <div v-else class="jbRmQG">
    ...
    <div class="hYLPFg">
      ...
      <div class="lfiYXW">
        ...
        <span
          v-if="form.outputToken.symbol"
        >{{ currentPoolSize.cny_amount }} CNY + {{ currentPoolSize.token_amount }} {{ form.outputToken.symbol }}</span>
        <span v-else>-</span>
      </div>
      <div class="lfiYXW">
        ...
        <span
          v-if="form.outputToken.symbol"
        >{{ yourPoolSize.cny_amount }} CNY + {{ yourPoolSize.token_amount }} {{ form.outputToken.symbol }}</span>
        <span v-else>-</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    btnDisabled() {
      ...
      if (this.isDelete) {
        if (
          this.yourPoolSize.cny_amount === 0 ||
          this.yourPoolSize.cny_amount < this.outputPoolSize.cny_amount
        ) {
          return true;
        }
        if (
          ...
          outputPoolSize.cny_amount === 0 ||
          outputPoolSize.token_amount === 0
        ) {
          return true;
        }
      } else {
  ...
  methods: {
    ...
    getOutputAmount(inputTokenId, outputTokenId, inputAmount) {
      ...
      this.$API
        .getPoolCnyToTokenPrice(inputTokenId, outputTokenId, _inputAmount)
        .then(res => {
          if (res.code === 0) {
    ...
    addLiquidity() {
      ...
      this.$API
        .wxpay({
          ...
          min_liquidity: utils.toDecimal(
            this.youMintTokenAmount,
    ...
    getYourMintToken(tokenId, amountBefore) {
      ...
      this.$API.getYourMintToken(tokenId, amount).then(res => {
        if (res.code === 0) {
          this.youMintTokenAmount = utils.fromDecimal(
            res.data,
            outputToken.decimals
          );
        }
    ...
    removeLiquidity() {
      ...
      const minCny = utils.toDecimal(
        this.calLimitValue(this.outputPoolSize.cny_amount),
        4
      );
      const minTokens = utils.toDecimal(
        this.calLimitValue(this.outputPoolSize.token_amount),
        4
      );
      this.$API
        .removeLiquidity({
    ...
    checkBalance(showError = true) {
      ...
      if (!this.isDelete) {
        ...
        if (!utils.isNull(outputToken) && outputToken.id !== 0) {
          if (
            parseFloat(this.yourPoolSize.cny_amount) <
            parseFloat(this.outputPoolSize.cny_amount)
          ) {
            if (showError) {
              this.$toast.fail({
                message: "余额不足",
