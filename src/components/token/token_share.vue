...
export default {
  ...
  computed: {
    ...
    link() {
      if (process.browser) {
        const slogan = [
          `${process.env.VUE_APP_URL}/token/${this.$route.params.id}?referral=${this.currentUserInfo.id}`,
          `${process.env.VUE_APP_URL}/user/${this.$route.params.id}?referral=${this.currentUserInfo.id}`,
